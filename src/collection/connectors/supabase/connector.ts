...
export const supabaseConnector: ConnectorDefinition = {
  ...
  optionalFields: [
    {
      ...
      description: "Required only for webhook mode. Send it in the x-moonarq-webhook-secret header.",
      required: false,
      secret: true,
      type: "password",
      placeholder: "Create a long random value",
    },
    {
  ...
  async sync(ctx) {
    ...
    if (ctx.trigger === "webhook") {
      if (!ctx.webhookPayload) throw new Error("Supabase webhook payload is missing.");
      const payloadHash = createHash("sha256").update(JSON.stringify(ctx.webhookPayload)).digest("hex");
      const profile = extractProfileRecord(ctx.webhookPayload);
      return {
        rawPayloads: [
          {
            externalId: `supabase-webhook-${payloadHash}`,
            fetchedAt,
            payload: ctx.webhookPayload,
            payloadHash,
            cursor: { fetchedAt, mode: "webhook" },
          },
        ],
        cursorAfter: { fetchedAt, mode: "webhook" },
        recordsFetched: profile ? 1 : 0,
        message: "Processed an authenticated Supabase public.profiles webhook.",
      };
    }
    const projectUrl = ctx.source.normalized_url ?? ctx.source.input_url;
    const serviceRole = ctx.credentials.service_role_key;
    ...
    if (!ctx.isDemoMode) {
      return {
        ...
        cursorAfter: { fetchedAt, mode: "webhook_only" },
        recordsFetched: 0,
        message: "No polling credential configured; awaiting authenticated Supabase webhooks.",
      };
    }
  ...
  getSetupInstructions() {
    return [
      ...
      "Send the encrypted per-source webhook secret in the x-moonarq-webhook-secret header (or Authorization: Bearer).",
