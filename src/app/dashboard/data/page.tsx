...
function hrefFor(params: Record<string, string | number | undefined>) {
  ...
  return `/dashboard/data${query ? `?${query}` : ""}`;
}
...
export default async function SourceDataExplorerPage({
  ...
  const tab = params?.tab ?? "website";
  const range = params?.range ?? "30d";
  const sourceId = params?.sourceId ?? "all";
  const page = Math.max(1, Number(params?.page ?? 1));
  const [sources, result] = await Promise.all([
    listSources(),
    getSourceDataExplorer({ tab, range, sourceId, page }),
  ]);
  ...
  return (
    <div className="mx-auto grid max-w-[1600px] gap-6">
      <SectionHeader
        ...
        title="Source Data Explorer"
        description="Inspect safe slices of Data Hub's own Supabase/Postgres data without leaving the command center. Rows are read-only, credentials are never shown, and all times are displayed in PT."
        action={
        ...
        <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
          <div className="flex flex-wrap gap-2">
            {tabs.map((item) => (
              <Link
                key={item.key}
                href={hrefFor({ tab: item.key, range, sourceId })}
      ...
      <GlassPanel className="overflow-hidden">
            ...
            <tbody>
              {result.rows.map((row) => (
                <tr key={row.id} className="border-t border-white/10">
                  {result.columns.map((column) => (
                    <td key={column} className="max-w-[18rem] truncate px-4 py-3 text-slate-300">
                      {row.cells[column] ?? ""}
                    </td>
                  ))}
                  <td className="px-4 py-3">
                    <CopyJsonButton value={row.json} />
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <div className="flex items-center justify-between border-t border-white/10 p-4">
          <LinkButton
            href={hrefFor({ tab: result.tab, range, sourceId, page: Math.max(1, result.page - 1) })}
            variant="secondary"
          ...
          <LinkButton
            href={hrefFor({ tab: result.tab, range, sourceId, page: result.page + 1 })}
            variant="secondary"
            className={!result.hasNextPage ? "pointer-events-none opacity-50" : ""}
          >
            Next
            <ArrowRight className="h-4 w-4" />
          </LinkButton>
