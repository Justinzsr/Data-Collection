  ...
  return (
    <div className="mx-auto grid max-w-[1500px] gap-6">
      <SectionHeader
        ...
        title="MoonArq Daily Report"
        description={`Yesterday, ${formatAppDate(startOfAppDateUtc(reportDate))} PT. Generated safely inside Data Hub; Excel is manual-only and authenticated.`}
        action={
          <>
            <form action="/api/reports/daily/generate" method="post">
            ...
            {report ? (
              <LinkButton href={`/api/reports/daily/excel?date=${reportDate}`} variant="secondary">
      ...
      {report ? (
        <>
          <GlassPanel className="p-4 sm:p-5">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              ...
              <Badge tone={report.run.health_status === "healthy" ? "green" : "amber"}>{report.run.health_status}</Badge>
            </div>
          </GlassPanel>
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  {(metricsBySection.get(section.section_key) ?? []).map((metric) => (
                    <div key={metric.id} className="rounded-lg border border-white/10 bg-black/20 p-3">
                      ...
                      <p className="mt-1 break-words text-lg font-semibold text-slate-100">
                        {displayMetric(metric.value, metric.text_value, metric.unit)}
                      </p>
                    </div>
                  ))}
                </div>
        ...
        <GlassPanel className="p-6">
          ...
          <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-400">
            Generate yesterday&apos;s report when you want a fixed, reviewable snapshot. The generator is idempotent, so rerunning it updates the same report date instead of creating duplicates.
          </p>
        </GlassPanel>
      )}
      ...
      <GlassPanel className="p-4 sm:p-5">
        ...
        <div className="grid gap-2">
          ...
          {reports.map((item) => (
            <Link
              key={item.id}
              href={`/dashboard/reports/daily?date=${item.report_date}`}
              className="flex flex-col gap-2 rounded-lg border border-white/10 bg-white/[0.03] p-3 transition hover:bg-white/[0.06] sm:flex-row sm:items-center sm:justify-between"
            >
