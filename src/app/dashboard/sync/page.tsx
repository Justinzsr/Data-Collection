      ...
      <div className="grid gap-3 lg:hidden">
        {health.recentRuns.map((run) => (
          <GlassPanel key={run.id} className="p-4">
            ...
            <p className="mt-2 text-sm text-slate-400">Started: {formatAppDateTime(run.started_at ?? run.created_at)}</p>
            <p className="text-sm text-slate-400">Duration: {run.duration_ms ? `${run.duration_ms}ms` : "-"}</p>
