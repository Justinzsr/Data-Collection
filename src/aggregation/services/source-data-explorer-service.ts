...
export type ExplorerTab = "website" | "supabase" | "sync_runs" | "raw_ingestions" | "metrics_daily" | "connector_events";
...
async function dbRows(tab: ExplorerTab, options: { range: AppDateRangeKey; sourceId?: string; page: number; pageSize: number }) {
  ...
  if (tab === "raw_ingestions") {
    ...
    values.push(options.pageSize + 1, offset);
    return queryRows(`select r.id, to_char(timezone('America/Los_Angeles', r.fetched_at), 'Mon DD, YYYY HH12:MI AM "PT"') as fetched_at_pt, r.source_type_key, coalesce(s.display_name,r.source_type_key) as source, coalesce(r.external_id,'') as external_id, r.payload_hash, left(r.payload::text,180) as payload_preview, r.payload as json from raw_ingestions r left join sources s on s.id=r.source_id where ${where.join(" and ")} order by r.fetched_at desc limit $${values.length - 1} offset $${values.length}`, values);
  }
  values.push(startOfAppDateUtc(range.startDate), endOfAppDateUtc(range.endDate));
  ...
  values.push(options.pageSize + 1, offset);
  return queryRows(`select e.id, to_char(timezone('America/Los_Angeles', e.created_at), 'Mon DD, YYYY HH12:MI AM "PT"') as created_at_pt, coalesce(s.display_name,'system') as source, e.event_type, e.severity, e.message, left(e.metadata::text,180) as metadata_preview, e.metadata as json from connector_events e left join sources s on s.id=e.source_id where ${where.join(" and ")} order by e.created_at desc limit $${values.length - 1} offset $${values.length}`, values);
}

function demoRows(tab: ExplorerTab, options: { range: AppDateRangeKey; sourceId?: string; page: number; pageSize: number }) {
  const store = getDemoStore();
  const names = sourceNames(store.sources);
  const range = rangeFor(options.range);
  const sourceId = selectedSource(options.sourceId);
  if (tab === "website") return paginate(store.webEvents.filter((event) => (!sourceId || event.source_id === sourceId) && event.occurred_at >= startOfAppDateUtc(range.startDate) && event.occurred_at <= endOfAppDateUtc(range.endDate)).map((event) => ({ id: event.id, occurred_at_pt: formatAppDateTime(event.occurred_at), event_name: event.event_name, path: event.path, referrer: event.referrer ?? "direct", country: event.country ?? "Unknown", device_type: event.device_type ?? "Unknown", browser_client: event.user_agent ?? "Unknown", source_mode: "Website Tracker", payload_hash: null, json: event.properties })), options.page, options.pageSize + 1);
  if (tab === "supabase" || tab === "metrics_daily") return paginate(store.metricsDaily.filter((metric) => (!sourceId || metric.source_id === sourceId) && metric.date >= range.startDate && metric.date <= range.endDate).filter((metric) => tab === "supabase" ? metric.source_type_key === "supabase" : true).map((metric) => ({ id: metric.id, date_pt: metric.date, source: names.get(metric.source_id ?? "") ?? metric.source_type_key, metric_key: metric.metric_key, metric_value: metric.metric_value, provider: typeof metric.dimensions.provider === "string" ? metric.dimensions.provider : "", users_total: metric.metric_key === "users_total" ? metric.metric_value : null, confirmed_users: metric.metric_key === "confirmed_users" ? metric.metric_value : null, sync_run_id: "", dimensions: preview(metric.dimensions), json: metric.dimensions })), options.page, options.pageSize + 1);
  if (tab === "sync_runs") return paginate(store.syncRuns.map((run) => ({ id: run.id, started_at_pt: formatAppDateTime(run.started_at), finished_at_pt: formatAppDateTime(run.finished_at), source: names.get(run.source_id ?? "") ?? run.source_type_key ?? "system", trigger: run.trigger, status: run.status, records_fetched: run.records_fetched, metrics_upserted: run.metrics_upserted, error_message: run.error_message ?? "", json: run.metadata })), options.page, options.pageSize + 1);
  if (tab === "raw_ingestions") return paginate(store.rawIngestions.map((item) => ({ id: item.id, fetched_at_pt: formatAppDateTime(item.fetched_at), source_type_key: item.source_type_key, source: names.get(item.source_id ?? "") ?? item.source_type_key, external_id: item.external_id ?? "", payload_hash: item.payload_hash, payload_preview: preview(item.payload), json: item.payload })), options.page, options.pageSize + 1);
  return paginate(store.connectorEvents.map((event) => ({ id: event.id, created_at_pt: formatAppDateTime(event.created_at), source: names.get(event.source_id ?? "") ?? "system", event_type: event.event_type, severity: event.severity, message: event.message, metadata_preview: preview(event.metadata), json: event.metadata })), options.page, options.pageSize + 1);
}

export async function getSourceDataExplorer(options: { tab?: string; range?: string; sourceId?: string; page?: number; pageSize?: number } = {}): Promise<ExplorerResult> {
  ...
  const rawRows = isRuntimeDatabaseConfigured() ? await dbRows(tab, { range, sourceId: options.sourceId, page, pageSize }) : demoRows(tab, { range, sourceId: options.sourceId, page, pageSize });
